...
package de.one_piece_api.screens.util;
...
public class GenericTabbedScreen extends Screen {
    ...
    public record TabData(String name, Screen screen, int width) {
    }

    public void addTab(String name, Screen screen, int tabWidth) {
    ...
    public void renderBackground(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (currentTab < tabs.size()) {
            tabs.get(currentTab).screen.renderBackground(context, mouseX, mouseY, delta);
        }
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (currentTab < tabs.size()) {
            tabs.get(currentTab).screen.renderWithTooltip(context, mouseX, mouseY, delta);
        }
    }
    ...
    public void resize(MinecraftClient client, int width, int height) {
        ...
        if (currentTab < tabs.size()) {
            tabs.forEach(tab -> tab.screen.resize(client, width, height));
        }
    }
    ...
    public void mouseMoved(double mouseX, double mouseY) {
        ...
        if (currentTab < tabs.size()) {
            tabs.get(currentTab).screen.mouseMoved(mouseX, mouseY);
        }
    }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
        if (currentTab < tabs.size()) {
            return tabs.get(currentTab).screen.mouseScrolled(mouseX, mouseY, horizontalAmount, verticalAmount);
        }
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
        if (currentTab < tabs.size()) {
            return tabs.get(currentTab).screen.mouseDragged(mouseX, mouseY, button, deltaX, deltaY);
        }
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        if (currentTab < tabs.size()) {
            return tabs.get(currentTab).screen.mouseReleased(mouseX, mouseY, button);
        }
    ...
    private void renderTab(DrawContext context, int x, int y,
        ...
        var text = Text.literal(tab.name).setStyle(Style.EMPTY.withFont(MyFonts.MONTSERRAT));
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (currentTab < tabs.size()) {
            Screen currentScreen = tabs.get(currentTab).screen;
